...
namespace Raidfelden.Services.Extensions.ImageSharp
{
    /// <summary>
    ...
    public class QueueLinearFloodFiller : AbstractFloodFiller
	{
		...
		protected bool CheckPixel(ref int x, ref int y)
		{
			...
			return ((pixel.R >= startColor.R - tolerance[0]) && (pixel.R <= startColor.R + tolerance[0]) &&
					(pixel.G >= startColor.G - tolerance[0]) && (pixel.G <= startColor.G + tolerance[0]) &&
					(pixel.G >= startColor.B - tolerance[0]) && (pixel.B <= startColor.B + tolerance[0]));
		}
